...
function renderClient() {
  ...
  $('#prodlist').innerHTML = state.stock
...
function renderCart() {
  ...
  wrap.innerHTML =
    ...
    '<th>Προϊόν</th><th class="num">Τεμ.</th><th class="num">Τιμή</th>' +
...
function addToCart() {
  ...
  if (!p) {
    ...
    msg.textContent = 'Διαλέξτε προϊόν από τη λίστα.';
    return;
  }
...
function submitOrder() {
  ...
  openModal(
    ...
    [
      ...
      { label: 'Αποστολή παραγγελίας', primary: true, onClick: finalizeOrder }
    ]
  );
}
      ...
      { label: 'Άκυρο', onClick: closeModal },
      {
        label: 'Έγκριση με αυτές τις αλλαγές', primary: true, onClick: () => {
          const decisions = shortages.map(s => ({
            ...s,
            choice: document.querySelector('input[name="sh' + s.idx + '"]:checked').value
          }));
          closeModal();
...
function approve(order, decisions) {
  ...
  if (!order.items.length) {
    ...
    order.log = log.concat('Η παραγγελία απορρίφθηκε: δεν έμεινε διαθέσιμο προϊόν.');
    save();
    render();
...
$('#prod').addEventListener('keydown', e => { if (e.key === 'Enter') addToCart(); });
$('#qty').addEventListener('keydown', e => { if (e.key === 'Enter') addToCart(); });
